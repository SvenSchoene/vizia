...
impl Event {
    ...
    /// Marks the event as synthetic (generated by the system rather than user input).
    ///
    /// Synthetic events are generated by systems like the layout system to trigger
    /// updates (e.g., hover state changes) after relayout. Handlers can check
    /// `meta.synthetic` to filter out these events when they only want to respond
    /// to real user input.
    pub fn synthetic(mut self, is_synthetic: bool) -> Self {
        self.meta.synthetic = is_synthetic;
        self
    }
...
pub struct EventMeta {
    ...
    /// Whether this event was generated synthetically by the system (e.g., layout system)
    /// rather than from real user input. Handlers can check this to filter out synthetic events.
    pub synthetic: bool,
}
